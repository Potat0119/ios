...
const TAB_KEYS = ['simbrief', 'List', 'Schedule', 'atis'] as const;
...
export default function Page() {
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col" style={{ ['--tab-h' as any]: '44px' }}>
      <Tabs
        items={panes.map(p => ({ key: p.key, label: p.label }))}
        activeKey={active}
        onChange={k => setActive(k as TabKey)}
      />
      <div className="relative w-full" style={{ height: 'calc(100dvh - var(--tab-h))', overflow: 'hidden' }}>
        {panes.map(p => (
          <section
            key={p.key}
            className={p.key === active ? 'relative h-full' : 'absolute inset-0 h-full opacity-0 pointer-events-none -z-10'}
          >
            {p.mode === 'iframe' && (
              <iframe src={p.url} title={p.label} style={{ width: '100%', height: '100%', border: 0, display: 'block' }} loading="eager" />
            )}
            {p.mode === 'component' && p.key === 'atis' && (
              <AtisSearch />
            )}
            {p.mode === 'component' && p.key === 'calculator' && (
              <Calculator />
            )}
          </section>
        ))}
